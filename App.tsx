...
import { 
  Sparkles, Search, Download, Upload, List, LayoutGrid, Archive, 
  Table, CloudUpload, Telescope, Zap, BarChart, Plus, Filter, 
  History, LayoutDashboard, Settings, Map as MapIcon, FileText,
  Sun, Moon, Shield
} from 'lucide-react';
...
const ServicesView = lazy(() => import('./src/components/ServicesView'));
...
const AppContent: React.FC = () => {
  const { 
    ...
    searchHistory, setSearchHistory, isAdmin
  } = useApp();

  const { saveSettings } = useApp();

  const [currentView, setCurrentView] = useState<'DASHBOARD' | 'SETTINGS' | 'LANDING' | 'TEMPLATES' | 'HISTORY' | 'FUNNEL' | 'ADMIN' | 'SERVICES'>('DASHBOARD');
  ...
  const handleExportCSV = () => {
      ...
      const headers = ['Nombre', 'Empresa', 'Cargo', 'Email', 'Teléfono', 'LinkedIn', 'Score', 'Estado', 'Industria', 'Ubicación'];
      ...
      try {
        ...
        const importedLeads: Lead[] = lines.slice(1).filter(line => line.trim()).map(line => {
            ...
            headers.forEach((header, index) => {
                ...
                if (header.toLowerCase().includes('nombre')) lead.name = val;
                else if (header.toLowerCase().includes('empresa')) lead.company = val;
                else if (header.toLowerCase().includes('cargo')) lead.role = val;
                else if (header.toLowerCase().includes('email')) lead.emailGuess = val;
                else if (header.toLowerCase().includes('tel')) lead.phone = val;
                else if (header.toLowerCase().includes('linkedin')) lead.linkedinUrl = val;
                else if (header.toLowerCase().includes('score')) lead.qualificationScore = Number(val) || 0;
                else if (header.toLowerCase().includes('industria')) lead.industry = val;
                else if (header.toLowerCase().includes('ubicaci')) lead.location = val;
            });
        ...
        if (importedLeads.length > 0) {
            ...
            setAppState(AppState.COMPLETE);
            addNotification(`${importedLeads.length} leads importados desde CSV`, "SUCCESS");
            updateGamification(10);
        }
      } catch (err) { 
          console.error(err);
          addNotification("Error al procesar CSV. Asegúrate de que el formato sea correcto.", "ERROR"); 
      }
    };
    reader.readAsText(file);
    e.target.value = '';
  ...
  if (!session && !isOfflineMode) {
      return <Auth onLoginSuccess={() => window.location.reload()} onOfflineClick={() => setOfflineMode(true)} />;
  }
  ...
  const SidebarButton = ({ icon: Icon, label, active, onClick, extraClass = "" }: any) => (
      ...
      >
          {active && <motion.div layoutId="sidebar-active" className="absolute left-0 top-0 bottom-0 w-1 bg-indigo-500 rounded-l-xl"></motion.div>}
  ...
  return (
    <div className={`h-screen flex font-sans overflow-hidden ${theme === 'dark' ? 'text-slate-100 bg-slate-950' : 'text-slate-900 bg-slate-50'}`}>
      ...
      {/* SIDEBAR */}
      <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-72'} glass-panel border-r-0 border-r border-white/5 flex flex-col hidden md:flex z-20 m-4 rounded-2xl transition-all duration-300 relative`}>
             <button 
                onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
                className="absolute -right-3 top-20 w-6 h-6 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-lg z-30 hover:bg-indigo-500 transition-colors"
             >
                {isSidebarCollapsed ? <Plus className="w-3 h-3 rotate-45" /> : <Plus className="w-3 h-3 rotate-0" />}
             </button>

             <div className={`p-6 border-b border-white/5 flex items-center gap-3 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                 <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/30 shrink-0">
             ...
             <div className="p-4 flex-1 overflow-y-auto space-y-8 scrollbar-hide">
                 <div className="space-y-1">
                     {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Principal</p>}
                     <SidebarButton icon={List} label={isSidebarCollapsed ? "" : "Dashboard"} active={currentView === 'DASHBOARD'} onClick={() => setCurrentView('DASHBOARD')} />
                     <SidebarButton icon={Filter} label={isSidebarCollapsed ? "" : "Pipeline"} active={currentView === 'FUNNEL'} onClick={() => setCurrentView('FUNNEL')} />
                     <SidebarButton icon={History} label={isSidebarCollapsed ? "" : "Historial"} active={currentView === 'HISTORY'} onClick={() => setCurrentView('HISTORY')} />
                     <SidebarButton icon={Zap} label={isSidebarCollapsed ? "" : "Catálogo Servicios"} active={currentView === 'SERVICES'} onClick={() => setCurrentView('SERVICES')} />
                 </div>

                 <div className="space-y-1">
                     {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Herramientas</p>}
                     <SidebarButton icon={LayoutDashboard} label={isSidebarCollapsed ? "" : "Generador Landing"} active={currentView === 'LANDING'} onClick={() => setCurrentView('LANDING')} />
                     <SidebarButton icon={FileText} label={isSidebarCollapsed ? "" : "Plantillas Email"} active={currentView === 'TEMPLATES'} onClick={() => setCurrentView('TEMPLATES')} />
                     <SidebarButton icon={Settings} label={isSidebarCollapsed ? "" : "Configuración"} active={currentView === 'SETTINGS'} onClick={() => setCurrentView('SETTINGS')} />
                     {isAdmin && (
                         <SidebarButton icon={Shield} label={isSidebarCollapsed ? "" : "Admin Panel"} active={currentView === 'ADMIN'} onClick={() => setCurrentView('ADMIN')} />
                     )}
                     <button onClick={() => setShowManualModal(true)} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 flex items-center gap-3 text-emerald-400 hover:bg-emerald-500/10 border border-transparent hover:border-emerald-500/20 mt-2 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                        <Plus className="w-4 h-4" />
                        {!isSidebarCollapsed && <span>Nuevo Lead</span>}
                     </button>
                 </div>
                 
                 {/* Filters Widget */}
                 {!isSidebarCollapsed && currentView === 'DASHBOARD' && (appState === AppState.COMPLETE || leads.length > 0) && (
                     <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-900/40 p-4 rounded-xl border border-white/5">
                         <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Filter className="w-3 h-3"/> Filtros Activos</h3>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <Suspense fallback={<LoadingSpinner />}>
            ...
            {currentView === 'LANDING' && <div className="h-full"><LandingGenerator addNotification={addNotification} apiKey={userProfile?.apiKey} userProfile={userProfile} saveSettings={saveSettings} /></div>}
            ...
            {currentView === 'ADMIN' && <div className="h-full overflow-y-auto pt-6"><AdminView /></div>}
            {currentView === 'SERVICES' && <div className="h-full overflow-y-auto pt-6"><ServicesView /></div>}
        </Suspense>
        ...
        >
                ...
                <header className="h-20 shrink-0 flex items-center justify-between px-8 z-20">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative group z-50">
                            <button className="glass-button p-2 rounded-lg text-slate-400 hover:text-white"><Archive className="w-5 h-5" /></button>
                            <div className="absolute right-0 top-full mt-2 w-56 glass-panel rounded-xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right">
                                <button onClick={handleCSVImportClick} className="w-full px-3 py-2 text-xs text-left text-slate-300 hover:bg-white/5 rounded-lg transition-colors flex items-center gap-2 mb-1"><Table className="w-3.5 h-3.5 text-emerald-400"/> Importar CSV</button>
                                <button onClick={handleExportCSV} className="w-full px-3 py-2 text-xs text-left text-slate-300 hover:bg-white/5 rounded-lg transition-colors flex items-center gap-2 mb-1"><Download className="w-3.5 h-3.5 text-emerald-400"/> Exportar CSV</button>
                                <button onClick={handleImportClick} className="w-full px-3 py-2 text-xs text-left text-slate-300 hover:bg-white/5 rounded-lg transition-colors flex items-center gap-2"><CloudUpload className="w-3.5 h-3.5 text-yellow-400"/> Restaurar JSON</button>
                                <button onClick={handleExportJSON} className="w-full px-3 py-2 text-xs text-left text-slate-300 hover:bg-white/5 rounded-lg transition-colors flex items-center gap-2"><Download className="w-3.5 h-3.5 text-indigo-400"/> Exportar JSON</button>
                            </div>
                        </div>
                        
                        {selectedLeadIds.size > 0 && (
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex items-center gap-1 bg-indigo-500/20 border border-indigo-500/30 px-3 py-1.5 rounded-xl">
                                <span className="text-xs font-bold text-indigo-300 mr-2">{selectedLeadIds.size}</span>
                                <button onClick={handleBulkDeepDive} className="p-1.5 hover:bg-indigo-500/20 rounded-lg text-indigo-300 transition-colors" title="Deep Dive Masivo"><Telescope className="w-4 h-4"/></button>
                                <button onClick={handleBulkCrmExport} className="p-1.5 hover:bg-indigo-500/20 rounded-lg text-indigo-300 transition-colors" title="Sync CRM"><CloudUpload className="w-4 h-4"/></button>
                            </motion.div>
                        )}
                        
                        {selectedLeadIds.size >= 2 && (<button onClick={() => setShowCompare(true)} className="glass-button px-3 py-2 rounded-xl text-xs font-bold text-white flex items-center gap-2"><BarChart className="w-4 h-4 text-purple-400" /> Comparar</button>)}
                        
                        <button onClick={toggleTheme} className="glass-button p-2 rounded-lg text-slate-400 hover:text-white transition-colors">
                            {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                        </button>

                        <div className="flex bg-white/5 rounded-xl p-1 border border-white/5">
                            <button onClick={() => setViewMode('BOARD')} className={`p-2 rounded-lg transition-all ${viewMode === 'BOARD' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}><LayoutGrid className="w-4 h-4"/></button>
                            <button onClick={() => setViewMode('LIST')} className={`p-2 rounded-lg transition-all ${viewMode === 'LIST' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}><List className="w-4 h-4"/></button>
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-8 scrollbar-thin">
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[80vh] animate-fadeIn">
                        <div className="text-center mb-10 max-w-2xl relative">
                            ...
                            <div className="flex gap-4 justify-center">
                                <button onClick={handleCSVImportClick} className="glass-button px-6 py-3 rounded-xl text-sm font-bold text-white flex items-center gap-2 hover:scale-105 transition-transform"><Table className="w-4 h-4 text-emerald-400"/> Importar CSV</button>
                                <button onClick={handleImportClick} className="glass-button px-6 py-3 rounded-xl text-sm font-bold text-slate-300 flex items-center gap-2 hover:scale-105 transition-transform"><Upload className="w-4 h-4"/> Restaurar Backup</button>
                            </div>
                        </div>
                        <LeadForm />
                    </div>
                ...
                {(appState === AppState.COMPLETE || (appState === AppState.IDLE && leads.length > 0)) && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pb-10 h-full flex flex-col gap-6">
