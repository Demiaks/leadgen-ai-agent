...
export interface Lead {
  ...
  // New: Buying Intent Prediction
  intentScore?: number; // 0-100
  intentReason?: string;

  // New: War Room Simulation
  warRoomSession?: WarRoomMessage[];

  // New: Discovery Call Feedback
  discoveryCallFeedback?: DiscoveryCallFeedback;
...
export type LandingStyle = 'CYBER' | 'MINIMAL' | 'CORPORATE';
...
export interface DeploymentConfig {
    type: 'SFTP' | 'FTP' | 'WEBHOOK' | 'NONE';
    host?: string;
    port?: number;
    username?: string;
    password?: string;
    secure?: boolean; // For FTP (FTPS)
    path?: string; // e.g. /public_html/
    webhookUrl?: string;
...
export interface AIService {
    ...
    monthlyCost: string;
    roi: string;
    recommendedProfile: 'DELEGADOR' | 'VISIONARIO' | 'TECNICO';
    scalingTip: string;
}
...
export interface WarRoomMessage {
    agentName: 'Investigador' | 'Copywriter' | 'Estratega';
    message: string;
    timestamp: number;
}

export interface DiscoveryCallFeedback {
    score: number;
    tone: string;
