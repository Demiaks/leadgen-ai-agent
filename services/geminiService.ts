...
export const generateLandingCopy = async (
    ...
    const colors = visualConfig || {
      ...
      1. Hero Section (Impacto inmediato):
         - Titular potente centrado en resultado/transformación.
         - Subtítulo que explique qué hace el producto/servicio y para quién es.
         - 3 beneficios clave en formato bullet (iconos check).
         - Botón CTA claro y directo ("Agendar Consultoría" o similar) apuntando a #contact-section.

      2. Identificación del problema:
         - Explicar los principales dolores del cliente ideal en el sector ${industry}.
         - Mostrar consecuencias de no resolverlo (pérdida de dinero, tiempo, competitividad).
         - Usar lenguaje claro y directo para generar urgencia estratégica.

      3. Presentación de la solución:
         - Explicar nuestro servicio como una solución estratégica.
         - Evitar tecnicismos innecesarios.
         - Describir cómo funciona en 3 pasos simples.
         - Enfocar siempre en beneficios, no en características.

      4. Beneficios claros y medibles:
         - Resultados concretos y transformación antes vs después.
         - Impacto en tiempo, dinero, esfuerzo o crecimiento.
         - Ventajas competitivas de nuestra solución de IA.

      5. Prueba social o credibilidad:
         - Testimonios o casos de éxito realistas para el sector ${industry}.
         - Datos o métricas que refuercen la confianza.

      6. Oferta clara:
         - Qué incluye exactamente el servicio.
         - Modalidad de acceso o compra.
         - Garantía o reducción de riesgo.

      7. Manejo de objeciones (FAQ):
         - Responder: ¿Es para mí?, ¿Cuánto tarda?, ¿Necesito experiencia?, ¿Qué pasa si no hay resultados?

      8. Cierre fuerte:
         - Reforzar el beneficio principal.
         - Repetir CTA claro apuntando a #contact-section.
         - Añadir urgencia si aplica.
      ...
      - Hazlo totalmente responsive.
      - Sección de Contacto Directo DEBE tener el id="contact-section".
      - Devuelve ÚNICAMENTE el código HTML, sin bloques de código markdown (\`\`\`html) ni explicaciones adicionales.
    `;
...
export const generateOrgChart = async (lead: Lead, apiKey?: string): Promise<OrgNode[]> => {
    ...
    const prompt = `Investiga la estructura organizacional de ${lead.company}. Identifica roles clave (CEO, CTO, Marketing, Ventas) y las personas que los ocupan actualmente.`;
                ...
                responseSchema: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      role: { type: Type.STRING, description: "Cargo que ocupa" },
                      department: { type: Type.STRING, description: "Departamento (ej. Ventas, IT, Dirección)" },
                      reportsTo: { type: Type.STRING, description: "ID del rol al que reporta (opcional)" },
                      type: { type: Type.STRING, description: "DECISION_MAKER, INFLUENCER, BLOCKER, or USER" }
                    },
                    required: ["id", "name", "role", "department", "type"]
                  }
                }
            }
        });
...
export const predictBuyingIntent = async (lead: Lead, apiKey?: string): Promise<{score: number, reason: string}> => {
    const ai = getAiClient(apiKey);
    if (!ai) return { score: 50, reason: "No se pudo analizar la intención." };

    const prompt = `Analiza la intención de compra de ${lead.company} para ${lead.valueProposition}. Busca noticias de expansión, rondas de inversión, o contrataciones recientes de perfiles como ${lead.role}.`;
    ...
    try {
        ...
        return JSON.parse(response.text || "{\"score\": 50, \"reason\": \"Error\"}");
    } catch {
        return { score: 50, reason: "Error en el análisis de intención." };
    }
};
