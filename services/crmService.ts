...
Lead calificado por IA. Score: ${lead.qualificationScore}.
Razón: ${lead.reasoning}

--- DATOS DE CONTACTO Y SOCIAL ---
LinkedIn: ${lead.linkedinUrl || 'No disponible'}
Website: ${lead.sourceUrl || 'No disponible'}
Teléfono: ${lead.phone || 'No disponible'}

--- CONTEXTO DE VENTAS ---
Pain Points: ${lead.painPoints?.join(', ') || 'No identificados'}
Tech Stack: ${lead.techStack?.join(', ') || 'No identificado'}
Señales de Compra: ${lead.buyingSignals?.map(s => s.type).join(', ') || 'Ninguna'}
Observación de Auditoría: ${lead.auditObservation || 'N/A'}
    `.trim();
...
export const exportToSalesforce = async (lead: Lead, token: string, instanceUrl: string): Promise<CrmResponse> => {
    ...
    const richDescription = `
Generated via AI Agent. Score: ${lead.qualificationScore}.
Insights: ${lead.reasoning}

--- CONTACT & SOCIAL DATA ---
LinkedIn: ${lead.linkedinUrl || 'N/A'}
Website: ${lead.sourceUrl || 'N/A'}

--- SALES CONTEXT ---
Pain Points: ${lead.painPoints?.join(', ') || 'None identified'}
Tech Stack: ${lead.techStack?.join(', ') || 'None identified'}
Buying Signals: ${lead.buyingSignals?.map(s => s.type).join(', ') || 'None'}
Audit Observation: ${lead.auditObservation || 'N/A'}
    `.trim();
...
export const syncLeadToCrm = async (lead: Lead, profile: UserProfile): Promise<Lead> => {
    ...
    if (profile.hubspotKey) {
        ...
        try {
            ...
            if (!response.ok && response.status !== 0) { // status 0 is for no-cors
            ...
            if (e.message.includes('Failed to fetch') || e.name === 'TypeError') {
                result = await simulateSync('Webhook');
            } else {
