    ...
    saveAll: async (leads: Lead[]) => {
        ...
        try {
            ...
            const rows = leads.map(lead => ({
                id: lead.id, // ID is TEXT in DB, so we can send whatever string
                user_id: user.id,
                name: lead.name,
    ...
    get: async (): Promise<UserProfile | null> => {
        ...
        try {
            ...
            const dbProfile = {
                ...
                savedLandingPages: data.settings?.savedLandingPages || []
            } as UserProfile;
        ...
        try {
            ...
            const updates = {
                ...
                settings: {
                    ...
                    savedLandingPages: profile.savedLandingPages || []
                },
                updated_at: new Date()
            };
            ...
            if (error) {
                ...
                if (error.code === 'PGRST204' || error.message?.includes('settings') || error.code === '42703') {
                    console.error("⚠️ Error de Schema en Base de Datos: Falta columna 'settings' o la tabla 'profiles' no existe. Ejecuta el script SQL de migración.");
                }
                throw error;
            }
