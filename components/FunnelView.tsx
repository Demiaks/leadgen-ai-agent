...
const FunnelView: React.FC<FunnelViewProps> = ({ leads }) => {
  ...
  const chartData = useMemo(() => {
      return [
          { name: 'Nuevos', count: stats.countNew, color: '#3b82f6' },
          { name: 'Contactados', count: stats.countContacted, color: '#f59e0b' },
          { name: 'Calificados', count: stats.countQualified, color: '#10b981' },
          { name: 'Descartados', count: stats.countDisqualified, color: '#ef4444' },
      ];
  }, [stats]);

  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 animate-fadeIn h-full flex flex-col overflow-hidden">
        <div className="glass-panel rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden border border-slate-700/50 flex flex-col h-full">
            ...
            {leads.length === 0 ? (
                ...
                <>
                <div className="flex-1 flex flex-col lg:flex-row items-center justify-between overflow-hidden gap-8">
                    
                    {/* FUNNEL CONTAINER - Fits available space */}
                    <div className="w-full lg:w-1/2 flex flex-col justify-center gap-0 relative flex-1">
                    ...
                    {/* CHART CONTAINER */}
                    <div className="w-full lg:w-1/2 h-64 lg:h-full min-h-[250px] bg-slate-900/50 rounded-xl p-4 border border-slate-800 flex flex-col">
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Distribución por Estado</h3>
                ...
                {/* METRICS FOOTER */}
                <div className="mt-6 w-full grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 shrink-0">
