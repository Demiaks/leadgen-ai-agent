...
import { 
  Target, Briefcase, MapPin, Search, Sparkles, CheckCircle, 
  Mail, Settings, Globe, Users, Hash 
} from 'lucide-react';
...
const LeadForm: React.FC = () => {
  const { handleSearch, appState, userProfile, searchLogs } = useApp();
  ...
  return (
    <motion.div 
      ...
      className="glass-panel rounded-3xl p-8 w-full max-w-2xl mx-auto shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden backdrop-blur-xl border border-white/10"
    >
      
      {/* Background Ambience */}
      <div className="absolute top-0 right-0 p-4 opacity-20 pointer-events-none">
        <Sparkles className="w-64 h-64 text-indigo-500 blur-sm" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        ...
        <div className="bg-black/20 p-1.5 rounded-xl flex gap-1 mb-6 border border-white/5 backdrop-blur-sm">
          {['WEB', 'MAPS', 'SOCIAL', 'COMPETITORS'].map((type) => (
             <button
              ...
              >
                {type === 'WEB' && <Briefcase className="w-3.5 h-3.5" />}
                {type === 'MAPS' && <MapPin className="w-3.5 h-3.5" />}
                {type === 'SOCIAL' && <Hash className="w-3.5 h-3.5" />}
                {type === 'COMPETITORS' && <Globe className="w-3.5 h-3.5" />}
        ...
        <div className="bg-white/5 p-5 rounded-2xl border border-white/5 space-y-4 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center relative z-10">
                <h3 className="text-xs font-bold text-indigo-300 uppercase tracking-widest flex items-center gap-2">
                    <CheckCircle className="w-4 h-4" /> Identidad del Agente
                </h3>
                {!userProfile && <span className="text-[10px] text-slate-500 flex items-center gap-1 cursor-pointer hover:text-white"><Settings className="w-3 h-3"/> Configura tu perfil</span>}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                ...
                <div className="md:col-span-2 space-y-1 pt-2">
                    <label className="text-[10px] font-bold text-indigo-300 uppercase tracking-widest flex items-center gap-1"><Mail className="w-3 h-3"/> Estrategia de Outreach</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wide"><Target className="w-4 h-4 text-indigo-400" /> {criteria.searchType === 'MAPS' ? 'Tipo de Negocio' : criteria.searchType === 'SOCIAL' ? 'Rol o Hashtag' : 'Persona Objetivo'}</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2"><label className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wide"><MapPin className="w-4 h-4 text-indigo-400" /> Ubicación</label><input type="text" name="location" placeholder="Ej. Madrid" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:bg-white/10 transition-all" value={criteria.location} onChange={handleChange} required /></div>
            <div className="space-y-2">
                <label className="text-xs font-bold text-slate-300 flex items-center justify-between uppercase tracking-wide"><span className="flex items-center gap-2"><Users className="w-4 h-4 text-indigo-400" /> Cantidad</span><span className="font-mono text-indigo-400 bg-indigo-500/10 px-2 rounded border border-indigo-500/20">{criteria.leadCount}</span></label>
        ...
        <div className="space-y-2"><label className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wide"><Sparkles className="w-4 h-4 text-indigo-400" /> Propuesta de Valor</label><textarea name="valueProposition" placeholder="Describe brevemente qué vendes y qué problema resuelves." className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white h-24 resize-none focus:outline-none focus:border-indigo-500 focus:bg-white/10 transition-all leading-relaxed" value={criteria.valueProposition} onChange={handleChange} required /></div>

        <motion.button 
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          type="submit" disabled={isLoading} className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-3 transition-all shadow-xl ${isLoading ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700' : 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-indigo-500/30 border border-indigo-500/50 hover:shadow-indigo-500/50'}`}>
          {isLoading ? (
             ...
             <><Search className="w-5 h-5" /> INICIAR BÚSQUEDA</>
          )}
        </motion.button>
        ...
        {isLoading && searchLogs.length > 0 && (
          <motion.div 
            ...
            className="mt-6 bg-black/40 border border-white/10 rounded-xl p-4 font-mono text-[10px] text-emerald-400 overflow-hidden"
          >
            <div className="flex items-center gap-2 mb-2 border-b border-white/5 pb-2">
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
              <span className="text-slate-400 uppercase tracking-widest">Agent Terminal Activity</span>
            </div>
            <div className="space-y-1 max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10">
              {searchLogs.map((log, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <span className={log.includes('ADVERTENCIA') ? 'text-amber-400' : ''}>{log}</span>
                </div>
              ))}
