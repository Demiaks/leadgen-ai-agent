...
import { Map as MapIcon, MapPin, Target } from 'lucide-react';
...
const MapView: React.FC<MapViewProps> = ({ leads, onSelectLead }) => {
    ...
    return (
        <motion.div 
          ...
          className="flex flex-col h-full bg-slate-950 overflow-hidden relative rounded-2xl border border-white/5 shadow-inner"
        >
            {/* Header Overlay */}
            ...
            {hoveredLead && (
                <motion.div 
                  ...
                  className="absolute bottom-8 left-1/2 glass-panel p-4 rounded-xl shadow-2xl z-30 min-w-[220px] border border-white/10"
                >
                    ...
                    <div className="flex items-center gap-1 mt-3 text-[10px] text-slate-400 border-t border-white/5 pt-2">
                        <MapPin className="w-3 h-3" /> {hoveredLead.address || hoveredLead.location}
                    </div>
                </motion.div>
            )}
        </motion.div>
