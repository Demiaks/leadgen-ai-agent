...
import { 
  X, CheckCircle, ArrowLeft, ArrowRight, ArrowUp, 
  Zap, Target, Cpu, MapPin, Sparkles 
} from 'lucide-react';
        ...
        >
            <div className="text-center max-w-md">
                <motion.div 
                    ...
                    className="bg-slate-800 p-6 rounded-full inline-flex mb-6 shadow-lg shadow-indigo-500/20"
                >
      ...
      return (
        ...
        >
            <motion.div 
                ...
                className="text-center max-w-md bg-slate-800/50 border border-slate-700 p-8 rounded-2xl shadow-2xl"
            >
        ...
        <div className="flex-1 flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <AnimatePresence mode="wait">
                <motion.div 
                    ...
                    transition={{ duration: 0.3 }}
                    className={`w-full max-w-2xl bg-slate-900 border-2 ${borderColor} rounded-3xl shadow-2xl p-8 relative`}
                >
                {/* Card Header */}
                ...
                <div className="space-y-6">
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                        <div className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><Sparkles className="w-3 h-3 text-indigo-400" /> El Análisis</div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <div className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><Target className="w-3 h-3 text-red-400" /> Dolores Detectados</div>
                        ...
                        <div>
                            <div className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><Cpu className="w-3 h-3 text-blue-400" /> Tech Stack</div>
                            <div className="flex flex-wrap gap-2">
                    ...
                    {currentLead.address && (
                        <div className="flex items-center gap-2 text-slate-500 text-sm">
                            <MapPin className="w-4 h-4" /> {currentLead.address}
                        </div>
                    )}
                </div>
            ...
            <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-6 md:hidden">
                <button onClick={() => handleAction(LeadStatus.DISQUALIFIED, 'left')} className="p-4 bg-red-600 rounded-full shadow-lg text-white"><X className="w-8 h-8" /></button>
                <button onClick={() => handleAction(LeadStatus.CONTACTED, 'up')} className="p-4 bg-yellow-500 rounded-full shadow-lg text-white"><ArrowUp className="w-8 h-8" /></button>
                <button onClick={() => handleAction(LeadStatus.QUALIFIED, 'right')} className="p-4 bg-emerald-600 rounded-full shadow-lg text-white"><CheckCircle className="w-8 h-8" /></button>
            </div>
