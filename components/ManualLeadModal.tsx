...
const ManualLeadModal: React.FC<ManualLeadModalProps> = ({ onClose, onSave }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newLead: Lead = {
            ...
            notes: formData.note ? [{ id: `note-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`, content: formData.note, createdAt: Date.now() }] : [],
            painPoints: [],
            techStack: [],
            outreach: { subjectVariants: [], email: "", linkedin: "", phone: "" },
            history: [{ id: `hist-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`, timestamp: Date.now(), action: 'Lead Created Manually', user: 'User' }]
        };
